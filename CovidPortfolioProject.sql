...
-- Looking at total_cases vs total_deaths in South Africa
-- Shows likelihood of dying if contracting covid in South Africa
...
FROM PortfolioProjectCovid.dbo.CovidDeaths
WHERE location = 'South Africa' 
--ORDER BY 1, 2 (cannot use this in a view)
...
SELECT location, population, MAX(total_cases) AS max_infection_count, CASE 
WHEN MAX(total_cases) = 0 THEN NULL
WHEN population = 0 THEN NULL
ELSE (MAX(total_cases) / population)*100
END AS max_infection_percentage
FROM PortfolioProjectCovid.dbo.CovidDeaths
WHERE continent <> ' ' 
...
WHERE continent <> ' ' 
GROUP BY location, population
-- ORDER BY max_infection_percentage DESC (cannot use this in a view)
...
WHERE continent <> ' ' 
GROUP BY location
-- ORDER BY total_death_count DESC (cannot use this in a view)

-- Countries with highest death count per continent
-- Note that continents are also found under location; it seems like the continents under location have more accurate numbers
-- When we get them from continent we say (where continent <> ' ') -- so we select continents, but not the blanks
-- When we get them from location, we say (where continent = ' ') -- so we select location, and where continents are blank
...
-- Also create view
-- We don't want to see 'World', 'International' or 'European Union'
...
SELECT date, SUM(new_cases) AS total_global_cases, SUM(new_deaths) AS total_global_deaths, CASE
...
SELECT SUM(new_cases) AS total_global_cases, SUM(new_deaths) AS total_global_deaths, CASE
WHEN SUM(new_cases) = 0 THEN NULL
WHEN SUM(new_deaths) = 0 THEN NULL
ELSE (SUM(new_deaths) / SUM(new_cases))*100
END AS percentage_total_global_deaths
FROM PortfolioProjectCovid.dbo.CovidDeaths
ORDER BY 1, 2
...
-- SUM new_vaccinations, and partition by location so that SUM starts over at each location -- As rolling_people_vaccinated
